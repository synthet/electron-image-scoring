...
When **project** **`fff-gallery`** MCP is connected (see [AGENTS.md § fff](../../../../AGENTS.md)), prefer `grep` / `find_files` / `multi_grep` for repeated repo search instead of unbounded shell grep loops. Use `patterns: string[]` and string `constraints` on `multi_grep` (never `queries` or array constraints). Still cap what you paste into agent responses.
