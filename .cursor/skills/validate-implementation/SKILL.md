...
- **Evidence or it didn't happen.** "Looks done", "should work", or "probably green" are not
  ...
  running the narrowest relevant test or command (`npm run test:run`, `npx tsc --noEmit`, `npm run lint`).
...
|----|-------------------|---------|----------|
| AC-1 | … | Verified | `npm run test:run` passed |
| AC-2 | … | Unknown  | needs Electron dev + backend API |
