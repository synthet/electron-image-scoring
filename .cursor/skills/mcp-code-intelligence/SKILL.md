...
```text
fff-gallery MCP (grep, find_files, multi_grep) + rg + fd + ast-grep + git tools + task runner (npm run / just)
```
...
|-------|----------|-----------|------|
| Indexed file search | **fff** MCP (`grep`, `find_files`, `multi_grep`) — project `fff-gallery` | Warm index, frecency, git-aware, typo-tolerant | Project MCP install; ~26 MB RAM / 14k files |
