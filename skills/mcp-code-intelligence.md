...
## Purpose
Compare MCP layers. Minimal: raw wrappers around rg, fd, read_file, git diff, and patch_file. Better: add ast-grep, git tools, and task-runner tools. Advanced: Serena or codebase-memory-mcp plus Zoekt and optional embeddings such as claude-context. Embedding-first indexing is heavier and should be secondary to text, structural, symbol, and graph search.
...
## MCP Comparison
- CLI wrappers: lowest memory, transparent, deterministic; expose `rg`, `fd`, bounded file reads, `git diff`, and patch application.
- Structural search: ast-grep MCP/CLI finds syntax shapes and supports controlled rewrites.
- Symbol search: universal-ctags or LSP-backed tools quickly locate definitions/references.
- Graph search: Serena and codebase-memory-mcp provide project memory, symbol/relationship navigation, and higher setup cost.
- Zoekt: fast trigram code index for large repos; good server/wrapper when repeated searches justify indexing.
- Embeddings: claude-context-style semantic search helps fuzzy discovery but can be memory/index heavy and less exact than text/AST search.
