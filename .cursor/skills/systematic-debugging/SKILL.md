...
description: >-
  Use when encountering a bug, failing test, build failure, unexpected behavior,
  flaky behavior, regression, performance anomaly, or integration issue. Apply
  before proposing fixes so the agent finds root cause, compares working patterns,
  tests one hypothesis at a time, and avoids guess-and-check patches. Also use when
...
For **pipeline / scoring / DB triage** on this product, prefer MCP (`image-scoring-mcp`) for live
system evidence; use this skill for **code-path** root cause.
...
- Use `test-driven-development` once the failing regression test defines the behavior to restore.
- Use `verification-before-completion` before claiming the bug is fixed.
- When the failure was non-obvious, add a short RCA entry if the repo documents a failure log.
