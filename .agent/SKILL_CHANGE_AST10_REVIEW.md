...
- [ ] **Canonical copy:** `.cursor/skills/<name>/SKILL.md` is **canonical**; `.claude/skills/<name>/SKILL.md` is regenerated in the **same PR** with `python scripts/sync_assistant_trees.py` — never hand-edited.
- [ ] **Inventory:** [SKILL_INVENTORY.md](./SKILL_INVENTORY.md) updated (new row or **Last reviewed** date).
- [ ] **Gates green:** `python scripts/sync_assistant_trees.py --check`, `python scripts/ci/check_agent_frontmatter.py`, and `python scripts/validate_cli_hub_skills.py` when a CLI-hub skill changed.

## Privilege and scope (AST03)

- [ ] **Scope:** Instructions stay within the skill's stated purpose (gallery renderer vs main process vs IPC vs docs); no broad "read all secrets" unless justified and called out.
...
- [ ] **Cross-repo:** Instructions that reach into `../image-scoring-backend` are read-only unless the ticket says otherwise.
...
- [ ] **Identity files:** Skill does not instruct writing to unrelated agent identity files; edits to repo `AGENTS.md` only when the ticket requires it and are explicit in the PR.
...
*Inventory:* [.agent/SKILL_INVENTORY.md](./SKILL_INVENTORY.md) · *Compilation:* [.agent/SKILL_COMPILATION.md](./SKILL_COMPILATION.md)
