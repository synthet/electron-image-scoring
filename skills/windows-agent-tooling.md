...
## Purpose
Windows host is best for editors, GitHub CLI, Docker Desktop, PowerShell scripts, simple search/editing, Node/pnpm, Python/uv, and desktop MCP clients. Keep repositories in WSL2 for Bash-heavy, Docker Compose, Unix-path MCP, and CI-like workflows.
...
## Recommended Split
- Windows host: VS Code/Cursor/Claude Desktop/ChatGPT, Docker Desktop, local LLM apps, GitHub CLI, simple PowerShell scripts.
- WSL2 Ubuntu: repositories, git, rg/fd/jq/yq, ast-grep, build tools, tests, MCP servers.
Native Windows is enough for simple search, editing, gh, uv Python scripts, Node/pnpm, and PowerShell automation. WSL2 is better for Bash-heavy repos, Docker Compose, monorepos, Unix-path MCP, and CI reproduction.
