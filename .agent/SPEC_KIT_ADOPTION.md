# Spec Kit concepts adopted by this repository

This project does not vendor GitHub Spec Kit, but it adopts the parts that fit this repository's existing agent workflow and Cursor-first asset model.
...
## When to require the extra gates

`/clarify`, `/tasks`, and `/analyze` are **required for non-trivial multi-AC work**. Trivial fixes may skip them with an explicit note in the plan or PR.
...
5. Regenerate mirrors after command or skill changes: edit `.cursor/` and `.agent/`, then run `python scripts/sync_assistant_trees.py`.

## Upstream

Cherry-picked from [synthet-code-framework](https://github.com/synthet/synthet-code-framework) (Cursor-first fork). See sibling backend [framework-adoption-port-manifest](https://github.com/synthet/image-scoring-backend/blob/main/docs/raw/framework-adoption-port-manifest.md).
