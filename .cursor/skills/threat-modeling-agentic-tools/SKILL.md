...
1. Read [.agent/SAFETY.md](../../../.agent/SAFETY.md) and backend `docs/technical/MCP_SEARCH_DISPATCH.md`.
2. **Enumerate assets:** secrets/tokens, credentials, workspace files, agent sessions, PostgreSQL data,
...
3. **Enumerate threats** (adapt to your surfaces):
   ...
   - Malicious tool caller → arbitrary operations? (mitigate: tool allowlist, `confirmed=True` gate, no raw shell tools)
...
- Renderer must not access DB or filesystem directly — IPC/preload only.
