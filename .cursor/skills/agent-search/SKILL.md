...
8. **fff MCP:** When **project** `fff-gallery` is connected, prefer `ffgrep`/`fffind` for repeated repo search; one-off probes may still use `rg`/`fd` — see [tool-selection.md](references/tool-selection.md) and [AGENTS.md § fff](../../../AGENTS.md).
