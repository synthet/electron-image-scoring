...
| **Project MCP** | `is-ui-mcp`, optional `fff-gallery` | [`.cursor/mcp.json`](../../../mcp.example.json) + `cd mcp-server && npm run build:registry` |
| **User MCP** | `github`, `subagent-orchestrator` | `~/.cursor/mcp.json` per [mcp.user.example.json](../../../mcp.user.example.json) — **not** fff |
| **IDE built-in** | Grep, SemanticSearch, Glob | Always available in Cursor; no install |
| **Project-local** | `eslint`, `vitest` | `npm install` in repo root |
...
> Run `fd --version`, `sg --version`, and `npm run doctor` in the project root.
...
**fff** must be configured in **project** `.cursor/mcp.json` with `cwd` set to this git repo — user-level config fails for repo-scoped indexing.

See [AGENTS.md § fff](../../../../AGENTS.md) and commented snippet in [mcp.example.json](../../../mcp.example.json).
...
When **image-scoring-backend** is a sibling checkout, log that CLI tools were provisioned:

```bash
python ../image-scoring-backend/scripts/agent-memory/log_session.py \
