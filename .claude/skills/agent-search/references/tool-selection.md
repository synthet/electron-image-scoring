...
| **fff MCP** (`grep`, `find_files`, `multi_grep`) | Repeated repo-wide file/content search when **project** `fff-gallery` MCP is connected | One-off bounded probe; fff not installed | Prefer over many grep tool roundtrips |
| **Graphify** (`graphify query` / `path` / `explain`) | Cross-module architecture; “how does X connect to Y”; god nodes — when `graphify-out/graph.json` exists | Literal string search; gallery IPC/backend triage (`is-ui-mcp`); no graph built yet | Escalate after `rg`/`fff` for structure |

## fff MCP (when connected)

When **fff** is registered as **`fff-gallery`** in **project** [`.cursor/mcp.json`](../../../mcp.example.json) with repo `cwd` (see [AGENTS.md § fff](../../../../AGENTS.md)):

- Prefer **`grep`** / **`find_files`** / **`multi_grep`** for repeated repo-wide search.
- Call shapes:
  - `grep({ query: "*.{ts,tsx} exif_transpose" })` — constraints are **inline** in `query` (no `constraints` param)
  - `find_files({ query: "…" })`
  - `multi_grep({ patterns: ["a", "b"], constraints: "*.{ts,tsx}" })` — `constraints` is a **string**
...
- Prefer MCP **`graphify-gallery`** tools (`query_graph`, `shortest_path`, …) when connected; else CLI **`graphify query`** / **`path`** / **`explain`**.
- Keep literal discovery as **`rg`** / **`fff`**; keep gallery IPC/backend triage on **`is-ui-mcp`**.
...
|----------|------|---------|
| Where is `ipcMain.handle` defined? | `rg` | `rg "ipcMain\.handle" electron/ -n` |
| All `.tsx` in components | `fd` | `fd -e tsx src/components` |
| Every exported function in a folder | `ast-grep` | `ast-grep --pattern 'export function $N($$$)' --lang ts src/` |
| What does `electron/` contain? | `tree` | `tree -L 2 electron/` |
| Find `config.json` key | `rg` or `jq` | `rg "database\.engine" .` |

Gallery search globs:

```bash
--glob '!node_modules' --glob '!dist' --glob '!mcp-server/dist'
```
