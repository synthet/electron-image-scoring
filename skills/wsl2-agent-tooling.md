...
## Purpose
WSL2 Ubuntu is the default for repositories, git, rg/fd/jq/yq, ast-grep, build tools, test commands, Docker Compose clients, and MCP servers expecting Unix paths. Keep files under the Linux filesystem, not /mnt/c, for speed.
...
## Recommended Split
- Store repositories in `~/src` inside WSL2.
- Use Windows for GUI/editor/desktop MCP clients; call WSL terminals for build/test.
- Avoid editing the same repo simultaneously from Windows and WSL if file watchers or line endings are unstable.
