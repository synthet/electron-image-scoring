#!/usr/bin/env python3
"""Compiled harness for verification-before-completion (image-scoring-gallery).
...
CLAIM_PROOF_CATALOG: dict[str, dict[str, str]] = {
    "tests_pass": {
        "claim": "Vitest unit suite passes",
        "proof": "npm run test:run",
        "not_enough": "Previous run or narrow file subset for a broad claim",
    },
    "tsc_renderer": {
        "claim": "Renderer TypeScript clean",
        "proof": "npx tsc --noEmit",
        "not_enough": "Assumed types from editor",
    },
    "tsc_electron": {
        "claim": "Electron main TypeScript clean",
        "proof": "npx tsc -p electron/tsconfig.json --noEmit",
        "not_enough": "Renderer-only tsc",
    },
    "lint_touched": {
        "claim": "ESLint on touched scope (or npm run lint)",
        "proof": "npm run lint",
        "not_enough": "Formatting only; known baseline debt is not a free pass for new errors",
    },
    "assets_synced": {
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.suite:
        ...
        result["notes"].append(
            "Pass --claim <id> and/or --suite; use --run to execute proofs."
        )
